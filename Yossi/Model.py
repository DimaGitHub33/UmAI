...
class Model():
    ...
    def fit(self):
        ...
        self.logger.debug('fit called with parameters conf={conf} '.format(conf=self.conf))
        ...
        if (not 'ColumnSelectionType' in conf or conf['ColumnSelectionType'] == None):
            self.logger.info('ColumnSelectionType not found in conf using default -> None')
            self.logger.info('ColumnSelectionType not found in conf using default for \"Keep\" -> []')
            self.logger.info('ColumnSelectionType not found in conf using default for \"Drop\" -> []')
        ...
        self.logger.info('Target  -> {Target}'.format(Target=Target))
        try:
            self.logger.info('dropping target {Target}'.format(Target=Target))
            Data = Data.drop(conf['Target'], axis=1)
        except Exception:

            self.logger.error("Target column is not exist")

        ## Drop ot Select columns from Data ----------------------------------------
        if (len(conf['ColumnSelectionType']) != 0 and len(conf['Keep']) != 0):
            try:
                self.logger.info('conf[\'Keep\']={columns}'.format(columns=conf['Keep']))
                Data = Data.loc[:, np.intersect1d(Data.columns.values, conf['Keep'])]
                self.logger.info('Selected chosen columns , columns={columns}'.format(columns=Data.columns))
            except Exception:
                self.logger.error("Didn't selected anything")
                raise

        if (len(conf['ColumnSelectionType']) != 0 and len(conf['Drop']) != 0):
            try:
                if(len(np.intersect1d(Data.columns.values,conf['Drop']))==0):
                    self.logger.error("Didn't drop any columns")         
                else:
                    self.logger.info('dropping columns using , conf[\'Drop\']={drop}'.format(drop=conf['Drop']))
                    Data = Data.drop(conf['Drop'],axis=1)
                    self.logger.info('Droped selected columns')
            except Exception:           
                self.logger.error("Didn't drop any columns")      
                raise  
