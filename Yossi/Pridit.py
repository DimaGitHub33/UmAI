...
class PriditClassifier():
    ...
    def Pridit(self):
        ...
        self.logger.debug('pridit called with parameters conf={conf} '.format(conf=self.conf))

        self.logger.info('check conf params ')
        ## Fill Configuration -----------------------------------------------------
        if (not 'UsingFactor' in conf):
            self.logger.info('UsingFactor not found in conf using default -> None')
            conf['UsingFactor'] = None
        if (not 'FactorVariables' in conf or conf['FactorVariables'] == None):
            self.logger.info('FactorVariables not found in conf using default -> []')
            conf['FactorVariables'] = []
            factorVariables = conf['FactorVariables']
        if (not 'NumericVariables' in conf or conf['NumericVariables'] == None):
            self.logger.info('NumericVariables not found in conf using default -> []')
            conf['NumericVariables'] = []
            numericVariables = conf['NumericVariables']
        if (not 'FactorsVariablesOrder' in conf):
            self.logger.info('FactorsVariablesOrder not found in conf using default -> None')
            conf['FactorsVariablesOrder'] = None
        if (not 'NumericVariablesOrder' in conf):
            self.logger.info('NumericVariablesOrder not found in conf using default -> None')
        ...
        self.logger.info("Removing all the unique columns in the data")
        for colName in Data.columns:
            if len(Data.loc[:,colName].unique())==1:
                self.logger.info(colName + " Removed")
                Data.drop(colName, inplace=True, axis=1)
        
        self.logger.info('execute pridit with conf -> {conf}'.format(conf=conf))

        
        ## Fill the FactorVariables and NumericVariables list for other columns in the input data ----
        if (conf['UsingFactor'] == 'Both'):
            self.logger.info('UsingFactor == Both using factors [FactorVariables + NumericVariables]'.format(conf=conf))
        ...
        if factorVariables is None or len(factorVariables) == 0:
            self.logger.info('factorVariables is None -> build auto factor variable'.format(conf=conf))
            factorVariables = []
            dataTypes = Data.dtypes.reset_index().rename(columns={'index': 'Index', 0: 'Type'})
            for Index, row in dataTypes.iterrows():
                if row['Type'] in ['object', 'str']:
        ...
        if numericVariables is None or len(numericVariables) == 0:
            self.logger.info('numericVariables is None -> build auto factor variable'.format(conf=conf))
            numericVariables = []
            dataTypes = Data.dtypes.reset_index().rename(columns={'index': 'Index', 0: 'Type'})
            for Index, row in dataTypes.iterrows():
                if row['Type'] in ['int64', 'float64']:
        ...
        self.logger.info('using factorsVariablesOrder with {conf}'.format(conf=factorsVariablesOrder))
        numericVariablesOrder = conf['NumericVariablesOrder']
        self.logger.info('using NumericVariablesOrder with {conf}'.format(conf=numericVariablesOrder))

        ## F calculation for Factor variables  ------------------------------------
        F = pd.DataFrame()
        for variableToConvert in factorVariables:
            # print(VariableToConvert)
            # TODO: TELL DIMA ABOUT THIS FIX
            if (variableToConvert in removed_columns):
                continue
        ...
        for variableToConvert in [NV for NV in numericVariables if NV not in factorVariables]:
            ...
            if numericVariablesOrder is None:
                ...
                if len(Order) == 0:
                    ...
                    # TODO: TELL DIMA ABOUT THIS CHANGE
                    df_order_variable = list(Order['Order'])
                    if df_order_variable[0] == 0:
