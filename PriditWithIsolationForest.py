...
class PriditAndIsolationForestClassifier():
    ...
    def Predict(self):
        ...
        numericvariableForIF = pd.DataFrame()
        for variableToConvert in [NV for NV in numericVariables if NV not in factorVariables]:
            ...
            numericvariableForIF[str(variableToConvert)] = variable['VariableToConvert']

        #Isolation Forest
        clf = IsolationForest(random_state = 0, n_jobs = 4,).fit(numericvariableForIF)
        isolationForestScores = clf.predict(numericvariableForIF)
        #pd.DataFrame(isolationForestScores).describe()
        ...
        return priditScore,F,firstEigenVector,isolationForestScores
